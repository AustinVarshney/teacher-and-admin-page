...
const TeacherAssignment: React.FC = () => {
  ...
  const handleDayToggle = (day: string) => {
    setSelectedDays(prev => {
      if (prev.includes(day)) {
        ...
        // Add day with default time - check if schedule already exists
        setDaySchedules(prevSchedules => {
          // Check if this day already has a schedule
          const exists = prevSchedules.some(s => s.day === day);
          if (exists) {
            // If it exists, don't add duplicate
            return prevSchedules;
          }
          // Add new schedule with default period 1
          return [
            ...prevSchedules,
            { day, period: 1, startTime: '09:00', endTime: '10:00' }
          ];
        });
        return [...prev, day];
      }
  ...
  const handlePeriodChange = (day: string, period: number) => {
    setDaySchedules(prev => prev.map(schedule => 
      schedule.day === day ? { ...schedule, period } : schedule
    ));
  };
  ...
  const handleNextStep = () => {
    ...
    if (currentStep === 1) {
      ...
      // Clean up: ensure daySchedules only contains selected days and no duplicates
      setDaySchedules(prev => {
        // Remove duplicates and ensure only selected days exist
        const uniqueSchedules = selectedDays.map(day => {
          const existing = prev.find(s => s.day === day);
          return existing || { day, period: 1, startTime: '09:00', endTime: '10:00' };
        });
  ...
  const renderStep3 = () => (
    <div className="step-content">
      <h3>Step 3: Set Time Schedules & Periods</h3>
      <p className="step-subtitle">Configure period number, start and end times for each selected day</p>
      
      <div className="schedules-list">
        {daySchedules.map((schedule, index) => (
          <div key={`schedule-${schedule.day}-${index}`} className="schedule-item">
            ...
            <div className="schedule-times">
              <div className="time-input-group">
                ...
                <select
                  ...
                  onChange={(e) => handlePeriodChange(schedule.day, Number(e.target.value))}
                  required
    ...
    <div className="teacher-assignment">
              ...
              <div className="summary-item">
                ...
                <div className="summary-schedules">
                  {daySchedules.map((s, index) => (
                    <div key={`${s.day}-${index}`} className="summary-schedule">
                      {s.day} - Period {s.period}: {s.startTime} - {s.endTime}
                    </div>
                  ))}
                </div>
              </div>
      ...
      <div className="info-box">
        ...
        <ul>
          ...
          <li>Step 3: Set period number (1-8) and specific start/end times for each selected day</li>
          <li>Periods help organize the daily schedule (e.g., Period 1: 9:00-10:00, Period 2: 10:00-11:00)</li>
